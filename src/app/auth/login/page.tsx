...
import { Film, Lock, Mail, AlertCircle, ArrowRight, Shield, User } from "lucide-react";

export default function LoginPage() {
  return (
    <Suspense fallback={<div className="mx-auto max-w-md px-4 py-20 text-center text-zinc-500">Loading sign in...</div>}>
      <LoginFormContent />
    </Suspense>
  );
}
    ...
    <div className="mx-auto flex min-h-[75vh] max-w-md flex-col justify-center px-4 py-12">
      <div className="rounded-3xl border border-zinc-800 bg-zinc-900/50 p-6 sm:p-8 backdrop-blur-xl shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold text-white tracking-tight">Sign In to CineBook</h1>
          <p className="text-xs text-zinc-400">
            Access your bookings, reserved seats, and digital tickets
          </p>
        </div>
        ...
        <div className="border-t border-zinc-800/80 pt-4 space-y-2.5">
          <span className="block text-[11px] font-bold text-zinc-400 uppercase tracking-wider text-center">
            1-Click Demo Sign In
          </span>
          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={() => handleQuickFill("alex@cinebook.com")}
              className="flex items-center justify-center gap-1.5 rounded-xl border border-zinc-800 bg-zinc-950 p-2 text-xs font-semibold text-zinc-300 hover:border-amber-500 hover:text-white transition-colors"
            >
            ...
            <button
              type="button"
              onClick={() => handleQuickFill("admin@cinebook.com")}
              className="flex items-center justify-center gap-1.5 rounded-xl border border-zinc-800 bg-zinc-950 p-2 text-xs font-semibold text-zinc-300 hover:border-amber-500 hover:text-white transition-colors"
            >
